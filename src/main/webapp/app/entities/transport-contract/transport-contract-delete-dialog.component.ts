...
export class TransportContractDeletePopupComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.activatedRoute.data.subscribe(({ transportContract }) => {
            setTimeout(() => {
                this.ngbModalRef = this.modalService.open(TransportContractDeleteDialogComponent as Component, {
                    size: 'lg',
                    backdrop: 'static'
                });
                this.ngbModalRef.componentInstance.transportContract = transportContract;
